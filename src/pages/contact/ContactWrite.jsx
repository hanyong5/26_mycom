...
export default function ContactWrite() {
  ...
  return (
    <div className="max-w-[800px] mx-auto px-5 md:px-10 py-10">
      ...
      <p className="text-sm text-gray-500 mb-8">궁금하신 점을 남겨주시면 빠르게 답변드리겠습니다.</p>
      <form onSubmit={handleSubmit} className="space-y-4">
        <Input label="제목" placeholder="문의 제목을 입력해주세요"
          value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))}
          error={errors.title} />
        <div className="flex flex-col gap-1">
          <label className="text-sm font-medium text-gray-700">내용</label>
          <textarea rows={10} placeholder="문의 내용을 자세히 입력해주세요"
        ...
        <div className="flex gap-2 justify-end pt-2">
          <Button type="button" variant="ghost" onClick={() => navigate('/')}>취소</Button>
          <Button type="submit" disabled={loading}>{loading ? '접수 중...' : '문의 접수'}</Button>
        </div>
      </form>
    </div>
  )
